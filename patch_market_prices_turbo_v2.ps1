# scripts/maintenance/update_market_prices_turbo.py
import os
...
def update_one_set(app, sc: str, sleep: float, max_age_days: int) -> Tuple[str, int]:
    # Cada hilo necesita su propio app context
...
def run(set_codes: Optional[List[str]], workers: int = 4, sleep: float = 0.0, max_age_days: int = 7):
    # Construye la app una vez y pásala a los threads
    app = None
    if create_app:
        try:
            app = create_app()
    ...
    # Lista de sets dentro de un contexto (principal)
    with app.app_context():
        if not set_codes:
            set_codes = list_set_codes()

    total = 0
    # Ejecuta los sets en paralelo
    with ThreadPoolExecutor(max_workers=max(1, workers)) as ex:
        futs = [ex.submit(update_one_set, app, sc, sleep, max_age_days) for sc in set_codes]
